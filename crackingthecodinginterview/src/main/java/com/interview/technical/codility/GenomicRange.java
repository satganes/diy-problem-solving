...
public class GenomicRange
{
    ...
    {
        int len = S.length();
        int[][] prefixSumArray = new int[len][4];
        int[] result = new int[P.length];

        for (int i = 0; i < len; i++)
        {
            char c = S.charAt(i);
            if (c == 'A') prefixSumArray[i][0] = 1;
            if (c == 'C') prefixSumArray[i][1] = 1;
            if (c == 'G') prefixSumArray[i][2] = 1;
            if (c == 'T') prefixSumArray[i][3] = 1;
        }
        // compute prefix sum
        for (int i = 1; i < len; i++)
        {
        ...
        for (int i = 0; i < P.length; i++)
        {
            ...
            for (int a = 0; a < 4; a++)
            {
                int subtractPreviousSum = 0;
                if (x - 1 >= 0) subtractPreviousSum = prefixSumArray[x - 1][a];
                if (prefixSumArray[y][a] - subtractPreviousSum > 0)
                {
                    result[i] = a + 1;
                    break;
                }
